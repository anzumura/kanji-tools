...
// 'Choice' is a class that supports presenting options and getting back a choice from user input. By
// default choices are read from terminal input (without requiring 'return'), but a istream can be passed
// in instead (good for testing). There is also support for 'default' choices (choice when just pressing
// 'return') and choice 'ranges' (see 'get' functions below).
class Choice {
public:
  ...
  // There is no 'quit' option by default, but it can be specified in the constructor or changed later
  // via 'setQuit' and 'clearQuit' methods. An 'istream' of '0' (the default) means read from stdin.
  Choice(std::ostream& out, OptChar quit = {}, const std::string& d = DefaultQuitDescription)
    : Choice(out, 0, quit, d) {}
  Choice(std::ostream& out, std::istream* in, OptChar quit = {}, const std::string& d = DefaultQuitDescription)
    : _out(out), _in(in) {
  ...
  // contains two or more consecutive values with empty descriptions then they will be displayed
  // as a range, i.e., 1-9, a-c, F-J, etc. - see ChoiceTest.cpp for examples of how to use this
  // class and expected output. 'useQuit' can be set to false to skip providing '_quit' value (has
  // no effect if '_quit' isn't set).
  ...
  auto get(const std::string& msg, const Choices& choices, OptChar def) const { return get(msg, true, choices, def); }
  auto get(const std::string& msg, const Choices& choices) const { return get(msg, choices, {}); }
  ...
  auto get(const std::string& msg, char first, char last, const Choices& choices, OptChar def) const {
    return get(msg, true, first, last, choices, def);
  }
  auto get(const std::string& msg, char first, char last, const Choices& choices) const {
    return get(msg, first, last, choices, {});
  }
  auto get(const std::string& msg, char first, char last) const { return get(msg, first, last, {}, {}); }
