...
public:
  ...
  ListQuiz(const QuizLauncher&, size_t question, bool showMeanings, const List&,
           KanjiInfo fields, size_t choiceCount, QuizStyle);
private:
  ...
  [[nodiscard]] size_t populateAnswers(const Entry&, const List& questions);
  ...
  [[nodiscard]] bool getAnswer(Choices& choices, bool& stopQuiz,
                               size_t correctChoice, const std::string& name);
  ...
  std::vector<size_t> _answers;

  const KanjiInfo _infoFields;
  const size_t _choiceCount;
