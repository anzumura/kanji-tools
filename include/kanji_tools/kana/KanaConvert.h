...
// mark (ー) is used (this can be overridded by a flag to produce the double vowel like おお).
...
// is accepted as input to the 'convert' function (when converting from Kana to Rōmaji the
...
public:
  ...
  // Support converting most non-alphanumeric ascii from narrow to wide values. These values are also used
  ...
  // separation handling after 'n' in Rōmaji output. Backslash maps to ￥ as per usual keyboard input.
  ...
  // versa (see KanaConvert.cpp 'Delimiters'). Also, when converting from Rōmaji, case is ignored so
  // both 'Dare' and 'dARe' would convert to 'だれ'. See 'ConvertFlags' in Kana.h for an explanation of
  // available flags that can be used. The third and fourth overloads update '_target' and '_flags'.
...
private:
  ...
  // 'insertUnique' performs an insert and ensures the value was added by using 'assert' - this can't
  // be done in a single line like 'assert(s.insert(x).second)' since that would result in the code
  // not getting executed when compiling with asserts disabled, i.e., a 'Release' build.
  ...
  // For input, either '_apostrophe' or '_dash' can be used to separate 'n' in the middle of Rōmaji
  // words like gin'iro, kan'atsu, kan-i, etc.. For Rōmaji output, '_apostrophe' is used. Note, dash
  // is used in 'Traditional Hepburn' whereas apostrophe is used in 'Modern (revised) Hepburn'.
  ...
  // with _apostrophe when producing Rōmaji output if they follow 'n'.
